...
1. **`:::info 【言語による違い】`**：プログラミング言語によって異なる可能性がある概念を示す
   - 本教材で学ぶ文法は全て基礎的な内容であり、ほとんどが他の言語でも共通の概念である
   - そのため、**言語によって異なる場合がある場合のみ**、この注釈を入れる
   - 例：「Javaではセミコロン`;`が必須だが、Pythonでは不要」
   - 例：「メソッドのオーバーロードはJavaやC++では可能だが、Pythonでは直接サポートされていない」
   - 受講生が他の言語を学ぶ際のヒントとなるよう、他言語での扱いについても簡潔に触れると望ましい

2. **`:::tip ポイント`**：学習のコツや重要なポイントを強調
   - 例：「Eclipseの補完機能を積極的に活用することで、コーディングが格段に効率的になる」
   - 実践的なアドバイスを提供
...
5. **`:::note 補足`**：技術的な背景や追加情報
   - 例：「`new`キーワードとインスタンスについては、次章で詳しく学ぶ」
   - 深い理解を促進
...
- **Java特有の仕様と、他言語共通の概念**を明示的に区別する（`:::info 【共通】`を活用）
