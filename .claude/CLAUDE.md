...
MDXファイルで演算子や特殊記号を記述する際は、バッククォートで囲むことを推奨する。
...
**適用ルール：**
- 箇条書きや見出しで演算子を列挙する場合は、各演算子をバッククォートで囲む
- 特に`<`、`>`、`&`などHTMLで特別な意味を持つ文字は必ずバッククォートで囲む
- 一貫性のため、同じ文脈内のすべての演算子を統一的にバッククォートで囲むことを推奨
