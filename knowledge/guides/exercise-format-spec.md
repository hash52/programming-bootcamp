...
5. **`## 解説` セクションを MDX 本体にベタ書きしてはならない。** 解説は必ず frontmatter の `explanation` フィールドに記述すること。MDX本体に `## 解説` が存在すると、採点前から解説が丸見えになる
6. **`<OneCompilerCodeBlock>` は「学習者が実際にコードを書く問題（コーディング問題）」専用** である。選択問題・穴埋め問題において、解説目的のコード例を表示したい場合は、**`explanation` フィールドに通常の Markdown コードブロック**（` ```java ... ``` `）として記述すること。MDX 本体に解説用の `<OneCompilerCodeBlock>` を埋め込んではならない
7. **穴埋め問題に限り**、問題文末尾に `**ヒント**：` のインラインテキストを記載できる。ただし、コラプシブルなヒント（「ヒントを表示」ボタンで展開）が必要な場合は frontmatter の `hint` フィールドに記述すること

> **⚠️ よくある間違い**: `## 解説` セクション内で `<OneCompilerCodeBlock>` を使用し、それごと MDX 本体に埋め込んでしまうパターンが起きやすい。解説はすべて `explanation` フィールドへ移し、コード例は通常の Markdown コードブロックに変換すること。
...
- **正答の位置（A/B/C/D）を偏らせない**: 同じ選択肢IDが正答として **3問以上連続しないよう** にする。「正答はいつもA」と学習者が推測できてしまうと、内容理解ではなくパターン学習が起きるため。1トピック内で A/B/C/D が概ね均等に分散するよう意識すること
...
- **コード例は通常の Markdown コードブロック**（` ```java ... ``` `）で記述すること。`<OneCompilerCodeBlock>` は React コンポーネントであり YAML フィールド内では使用できない
...
- [ ] 紛らわしい誤答が含まれているか
- [ ] 同じ正答ID（A/B/C/D）が3問以上連続していないか（同トピック内）
...
- [ ] 本体に `## 解説` セクションがないか（解説は frontmatter の `explanation` に記述すること）
- [ ] 本体に `## 問題` 見出しがないか（問題文を直接記述すること）
- [ ] import文が frontmatter 直後に配置されているか
- [ ] 選択問題・穴埋め問題の MDX 本体に `<OneCompilerCodeBlock>` が埋め込まれていないか（解説目的での使用禁止。コード例は `explanation` 内に Markdown コードブロックで記述すること）
- [ ] `explanation` フィールド内のコード例が `<OneCompilerCodeBlock>` ではなく通常の Markdown コードブロックになっているか
