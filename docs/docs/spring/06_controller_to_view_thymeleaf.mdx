...
Thymeleaf（タイムリーフ）は、Spring Bootのデフォルトビューエンジンである。
...
- 値の出力には `th:text` や `[[...]]` を用いる。`th:utext` はXSSリスクがあるため原則使わない
...
<QuestionList topicId="06_controller_to_view_thymeleaf" category="spring" />
