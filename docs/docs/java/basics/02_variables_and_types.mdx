...
Javaには、以下のような基本データ型がある：

| 型 | 扱えるデータ | 例 |
|------|--------------|-----|
| **int** | 整数 | `10`, `-5`, `0` |
| **double** | 小数 | `3.14`, `-0.5`, `2.0` |
| **boolean** | 真偽値（trueまたはfalse） | `true`, `false` |
...
**注意:**
`String`は **大文字のS** で始まる。他の型（int, double, boolean）は小文字で始まるが、Stringは例外である。
...
- 最初の単語は小文字、2つ目以降の単語の最初は大文字
- 例：`firstName`、`totalPrice`、`isAdult`
...
- **変数の命名規則**：キャメルケースを使い、わかりやすい名前をつける
- **型変換**：
  - 小さい型 → 大きい型：自動変換
