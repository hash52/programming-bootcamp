...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("配列の要素: " + numbers[0]);
        System.out.println("配列の要素: " + numbers[1]);

        // 範囲外アクセス → プログラムが停止
        System.out.println("配列の要素: " + numbers[10]);
...
public class Main {
    public static void main(String[] args) {
        ...
        try {
            ...
            System.out.println("エラー: 配列の範囲外にアクセスしようとしました");
...
<OneCompilerCodeBlock
  language="java"
  code={`// 実用例：ユーザー入力を安全に処理
public class Main {
    public static void main(String[] args) {
        String[] inputs = {"10", "20", "abc", "30"};  // 3番目は数値ではない
        ...
        for (String input : inputs) {
            try {
                ...
                sum += number;
                successCount++;
                System.out.println(input + " を数値に変換しました");

            } catch (NumberFormatException e) {
                System.out.println(input + " は数値ではありません（スキップ）");
            }
        }
...
:::info 検査例外は実務で頻繁に遭遇する
検査例外（`IOException`, `SQLException`など）は、**実務で必ず遭遇する例外** である。
