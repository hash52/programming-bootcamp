...
public class Main {
    ...
    // ... さらに続く（実際のプロジェクトでは1000行以上になることも）
}`}
/>
...
以下は、計算用のメソッドを`MathUtils`クラスに分けた例である。  
**実際には別ファイルになるが、ここでは実行のために1つのコードブロックにまとめている。**

<OneCompilerCodeBlock
  codeId="44f6d85xr"
  language="java"
  code={`// MathUtils.java（計算用のメソッドをまとめた別ファイル）
class MathUtils {
    public static int add(int a, int b) {
        return a + b;
    }
...
**`Ctrl + クリック`**（Macは`Cmd + クリック`）で、メソッドやクラスの定義へジャンプできる。

**使い方:**
1. メソッド名やクラス名の上で`Ctrl`キーを押しながらクリック
...
```
→ `MathUtils.java`の`add`メソッドの定義にジャンプする
...
<QuestionList topicId="08_multiple_classes" category="java/basics" />
